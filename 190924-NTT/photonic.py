...
def racetrack(radius = 10, width = 0.5, lc = 5, angle_resolution = 2.5, layer = 0):
    ...
    angle.sort()
    t=angle*np.pi/180
    inner_points_x = [i+np.sign(i)*lc/2 for i in inner_radius*np.cos(t)]
    inner_points_y = (inner_radius*np.sin(t)).tolist()
    outer_points_x = [i+np.sign(i)*lc/2 for i in outer_radius*np.cos(t)]
    outer_points_y = (outer_radius*np.sin(t)).tolist()
    xpts = inner_points_x + outer_points_x[::-1]
    ypts = inner_points_y + outer_points_y[::-1]
    D.add_polygon(points = (xpts,ypts), layer = layer)
...
def arc_grating(num_periods = 20, period = 0.75, fill_factor = 0.5, angle = 45, length_taper = 5, width = 0.5, layer = 0):
    ...
    for i in range(num_periods):
        cgrating = G.add_ref(
            arc(radius=cradius,
                ...
                layer = layer, 
                angle_resolution=1*(length_taper/cradius) 
                ))
        cradius += period
    ...
    B = arc(radius=length_taper/2,start_angle=-angle/2,theta=angle,width=length_taper,layer=layer,angle_resolution=1)
...
def archimedes(bent = 20, width = 0.5, n = 1, distance = 10, angle_resolution = 1, layer = 1):
    ...
    inner_sp_x = (inner_rho*np.cos(sp_t)).tolist()
    inner_sp_y = (inner_rho*np.sin(sp_t)).tolist()
    outer_sp_x = (outer_rho*np.cos(sp_t)).tolist()
    outer_sp_y = (outer_rho*np.sin(sp_t)).tolist()
    # arc section
    ...
    outer_arc_y = ( arc_radius*np.cos(inner_t) + (arc_radius+width*0.5)*np.sin(arc_t) ).tolist()
    ...
    outer_ext_y = ( ext_radius*np.cos(outer_t) + (ext_radius+width*0.5)*np.sin(ext_t) ).tolist()
